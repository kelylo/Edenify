...
const Home: React.FC = () => {
  ...
  const refreshOperationalStatus = useCallback(async (withFeedback: boolean) => {
    ...
    try {
      const statusResponse = await fetch(API_BASE + '/api/telegram/status');
      ...
      if (telegramChatId) {
        ...
        const tasksResponse = await fetch(`${API_BASE}/api/telegram/tasks/${encodeURIComponent(telegramChatId)}`);
