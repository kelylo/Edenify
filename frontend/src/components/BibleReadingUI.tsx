...
import { CheckCircle2, Circle, BookOpen, Volume2, Square } from 'lucide-react';
...
export const BibleReadingUI: React.FC<BibleReadingUIProps> = ({
  ...
  const toggleSpeech = () => {
    if (!reading || !reading.fullText) return;
    if (isSpeaking) {
    ...
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance("Today's reading. " + reading.passages + ". " + reading.fullText);
      ...
      <div className="rounded-2xl border-2 border-primary/30 bg-gradient-to-br from-primary/5 to-transparent p-6 shadow-sm">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className={cn(
              "text-[10px] font-bold uppercase tracking-[0.12em] px-3 py-2 rounded-full transition-colors flex items-center gap-2",
              isSpeaking ? "bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white" : "bg-primary/10 text-primary hover:bg-primary hover:text-white"
            )}
          >
