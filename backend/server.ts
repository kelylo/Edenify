...
function buildTelegramHealthSummary(tokenConfig: { token: string; source: string }, botStatus: any, webhookStatus: any, linkedChats: number, reminderCount: number, lastPollAt: string, lastPollError: string) {
  const issues: string[] = [];

  if (!tokenConfig.token) {
    issues.push('Telegram bot token is missing from Railway environment variables.');
  } else if (!botStatus.tokenValid) {
    issues.push('Telegram bot token is present but Telegram rejected it.');
  }

  if (webhookStatus?.url) {
    issues.push('A Telegram webhook is configured; polling mode requires it to be cleared.');
  }

  if (!linkedChats) {
    issues.push('No Telegram chat is linked yet.');
  }

  if (lastPollError) {
    issues.push('The poller reported a recent error.');
  }

  const healthy = issues.length === 0;
  return {
    healthy,
    mode: webhookStatus?.url ? 'webhook' : 'polling',
    tokenConfigured: Boolean(tokenConfig.token),
    ...
    recommendedAction: healthy
      ...
      : 'Check the issues above, then restart Railway after updating the Telegram token or clearing the webhook.',
  };
}
...
async function startServer() {
  ...
  app.get('/health', async (req, res) => {
    try {
      ...
      const botStatus = await getTelegramBotStatus(tokenConfig.token);
      res.json({ status: 'OK', telegram: botStatus });
    } catch (e: any) {
  ...
  app.use((req, res, next) => {
    ...
    const allowOrigin = allowAll || corsOrigins.includes(origin);
  ...
  app.get('/api/telegram/status', async (req, res) => {
    ...
    const reminderCount = db.telegram?.reminders ? Object.keys(db.telegram.reminders).length : 0;
    const botStatus = await getTelegramBotStatus(token);
    const webhookStatus = await getTelegramWebhookStatus(token);
    const health = buildTelegramHealthSummary(
      tokenConfig,
      botStatus,
      webhookStatus,
      linkedChats,
      reminderCount,
      db.telegram?.lastPollAt || '',
